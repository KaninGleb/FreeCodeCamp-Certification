...
// You need to set up your component to dynamically render the correct number of list elements long before
// someone using the program decides that today is laundry day.
...
// userInput should be initialized as an empty string, and toDoList should be initialized as an empty array.
...
//
// Note: You may know that all sibling child elements created by a mapping operation like this do need to be supplied
// with a unique key attribute. Don't worry, this is the topic of the next challenge.
...
class MyToDoList extends Component {
  ...
  render() {
    const items = this.state.toDoList.map((item, index) => {
      return <li key={index}>{item}</li> // Change this line
    })
