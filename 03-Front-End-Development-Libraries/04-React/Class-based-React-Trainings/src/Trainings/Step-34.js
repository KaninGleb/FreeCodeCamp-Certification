// ❓DESCRIPTION
// The componentDidMount() method is also the best place to attach any event listeners you need to
// add for specific functionality.
...
// 🎯 Assignment
// Attach an event listener in the componentDidMount() method for keydown events and have these events
// trigger the callback handleKeyPress(). You can use document.addEventListener() which takes
// the event (in quotes) as the first argument and the callback as the second argument.
//
...
// It's good practice to use this lifecycle method to do any clean up on React components before
// they are unmounted and destroyed. Removing event listeners is an example of one such clean up action.
