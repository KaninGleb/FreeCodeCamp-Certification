...
// state 3,state 4, ... and so on where each state may be similar to the last, but each is a distinct piece of data.
...
// Fortunately, JavaScript (especially ES6) provides several useful tools you can use to enforce the immutability of
// your state, whether it is a string, number, array, or object.
// Note that strings and numbers are primitive values and are immutable by nature. In other words, 3 is always 3.
// You cannot change the value of the number 3. An array or object, however, is mutable.
...
// See if you can find a way to return a new array with the item from (action.to do) appended to the end.
